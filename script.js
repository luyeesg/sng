...
const first = ['Baby Oil', 'Bad News', 'Big Burps', "Bill 'Beenie-Weenie'",
...
const last = ['Appleyard', 'Bigmeat', 'Bloominshine', 'Boogerbottom', 
...
'Pieplow', 'Pinkerton', 'Porkins', 'Putney', 'Quakenbush', 'Rainwater', 
'Rosenthal', 'Rubbins', 'Sackrider', 'Snuggleshine', 'Splern', 'Stevens', 
...
button.addEventListener("click", () => {
    ...
    let randomFirst = Math.floor(Math.random() * first.length)
    let randomLast = Math.floor(Math.random() * last.length)

    answer.textContent = `${first[randomFirst]} ${last[randomLast]}`
})